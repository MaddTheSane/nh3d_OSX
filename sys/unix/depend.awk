...
#
# usage:
#   cd src ; nawk -f depend.awk ../include/*.h list-of-.c/.cpp-files/.m
#
...
{
  if (file ~ /\.cp*|\.m$/) {
    #prior to very first .c|.cpp|.m file, handle some special header file cases
    if (!c_count++)
      output_specials()
    #construct object filename from source filename
    targ = file;  sub("^.+/", "", targ);  sub("\\.cp*$|\\.m", ".o", targ)
...
function format_dep(target, source,		n, i, list)
{
  ...
  if (source ~ /\// && substr(source, 1, 11) != "../include/") {
    ...
    else if (source ~ /\.m$/)		# "../win/nh3d/foo.m"
      print "\t$(CC) $(OBJCFLAG) $(NH3DINC) -c -o $@ " source
    else
