...
final class NH3DOpenGLView: NSOpenGLView {
    ...
    private func timerFired(_ sender: AnyObject) {
        ...
        openGLContext?.setValues(&vsType, for: NSOpenGLContext.Parameter.swapInterval)
    ...
    */
    ...
    @objc private func defaultsDidChange(notification: NSNotification?) {
        ...
        if !hasWait {
            ...
            oglFrameRateMenu?.item(withTag: 1004)?.state = NSControl.StateValue.offState
            oglFrameRateMenu?.item(withTag: 1005)?.state = NSControl.StateValue.offState
            oglFrameRateMenu?.item(withTag: 1006)?.state = NSControl.StateValue.offState
        } else if OPENGLVIEW_WAITRATE == WAIT_FAST {
            waitRate = WAIT_FAST
            oglFrameRateMenu?.item(withTag: 1004)?.state = NSControl.StateValue.onState
            oglFrameRateMenu?.item(withTag: 1005)?.state = NSControl.StateValue.offState
            oglFrameRateMenu?.item(withTag: 1006)?.state = NSControl.StateValue.offState
        } else if OPENGLVIEW_WAITRATE == WAIT_NORMAL {
            waitRate = WAIT_NORMAL
            oglFrameRateMenu?.item(withTag: 1004)?.state = NSControl.StateValue.offState
            oglFrameRateMenu?.item(withTag: 1005)?.state = NSControl.StateValue.onState
            oglFrameRateMenu?.item(withTag: 1006)?.state = NSControl.StateValue.offState
        } else {
            waitRate = WAIT_SLOW
            oglFrameRateMenu?.item(withTag: 1004)?.state = NSControl.StateValue.offState
            oglFrameRateMenu?.item(withTag: 1005)?.state = NSControl.StateValue.offState
            oglFrameRateMenu?.item(withTag: 1006)?.state = NSControl.StateValue.onState
        }
    ...
    @IBAction func changeWaitRate(_ sender: NSMenuItem) {
        ...
        case 1003: // no wait
            ...
            sender.menu?.item(withTag: 1004)?.state = NSControl.StateValue.offState
            sender.menu?.item(withTag: 1005)?.state = NSControl.StateValue.offState
            sender.menu?.item(withTag: 1006)?.state = NSControl.StateValue.offState
        ...
        case 1004:
            ...
            sender.menu?.item(withTag: 1003)?.state = NSControl.StateValue.offState
            sender.menu?.item(withTag: 1005)?.state = NSControl.StateValue.offState
            sender.menu?.item(withTag: 1006)?.state = NSControl.StateValue.offState
