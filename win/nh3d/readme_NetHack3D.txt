...
・良くある質問として、複数持っている物を数を指定して処理する場合は、コマンドのあとに数量を指定してください。
（例：矢（fに割り当てられていたとします。）を3個置きたい場合、d 3 fのように入力してください。numpadがオンの場合、数字入力時に
GUIが反応すると思いますが、問題ありません。）
...
2006/05/27 version 2.0.1
・トラディショナルモード時の描画処理を大幅に高速化。
・トラディショナルASCIIモード時、12ポ以上のフォントでシャドウを付けるようにした。
・トラディショナルモード時、カーソルキー←→で左右移動できるようにした。

2006/05/23 version 2.0.0
・全体マップの拡大・縮小機能を追加。
・サウンドミュート機能を追加。
・トラディショナルASCIIモードを追加。
・トラディショナルタイルモードを追加。
・トラディショナルモード用の設定項目を追加。
---　タイル画像ファイルは、jpg,png,bmp,tiffファイルで、1列あたりのタイル数（20〜40）タイル列の数（20〜40）
---　を指定することでいくつかのタイルフォーマットに対応しています。タイル1個あたりのサイズは自動的に判別されます。
---　また、タイル固有のサイズとは別に表示上のサイズを高さ、幅、それぞれに8ドット〜128ドット（8ドット刻み）の範囲で指定可能です。
---　実用的なのは16〜32位だと思います。全体マップとうまく併用してみてください。ASCIIモード時はマップフォントのサイズで
---　描画域が決定されます。


2006/05/10 version1.3.2
